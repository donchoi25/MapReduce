...
class MR_Framework ():
    ...
    def init_server (self):
        ...
        try:
            ...
            # base port + 1 to push args to reduce workers
            # base port + 3 to pull barrier signals indicating workers are up
            # base port + 4 to pull results from map workers
            # base port + 5 to pull results from reduce workers
    ...
    def distribute_map_tasks (self):
        ...
        try:
            ...
            # open database from couchdb
            couch = couchdb.Server('http://admin:password@129.114.27.202:30001')
            db = couch['topics']

            # get rows from database
            rows = db.view('_all_docs').rows
            num_rows = len(rows)

            # find number of rows to send to map workers
            rows_per_map = int (round (num_rows/self.M))
            print("number of rows = ", num_rows, ", rows per map = ", rows_per_map)
        
            print(("MR::solve - master sending args to {} map workers:".format(self.M)))
            for i in range (self.M):

                start = rows_per_map * i
                stop = rows_per_map * (i+1)

                # Store the rows to send to the map worker in content
                content = []
                if (i == self.M-1):
                    stop = num_rows
...
##            mf.flush ()
    ...
    def solve (self):
        ...
        try:
            ...
            metricsfile.write("Map phase\tShuffle Phase\tReduce Phase\tFinalize\tTotal\n")
            # run the iterations
